...
orderRequestForm.onsubmit = async (e) => {
  e.preventDefault();
  ...
  const response = await fetch("http://localhost:3000/api/order/create", {
    ...
    body: JSON.stringify(Object.fromEntries(new FormData(orderRequestForm))),
  });

  const result = await response.json();

  const receipt = `<div id="my-order" >
                       <div class="row">
                            <div class="col-md-4 col-sm-6">
                                <div class="sf-team">
                                    ...
                                    <div class="text-col">
										...
										<p>
												CLIENT NAME:  ${result.fullName},
										<br />
										
										ORDERED ITEM: ${result.foodName} <br/>
										PRICE: UGX ${result.price}<br/>
										QTY:  ${result.quantity}
										<br />
										 Order ID is ${result._id}
										<br />
										Thanks
										</p>
                                 <ul class="team-social">
                                            <li id=""><a href="/"><i class="fas fa-times-circle" aria-hidden="true"></i></a></li>
...
var form = document.getElementById('orderRequestForm');
form.reset
